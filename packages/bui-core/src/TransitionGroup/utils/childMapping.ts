 ...
 */
export const getChildMapping = (children, mapFn?) => {
  const mapper = (child) =>
...
export const mergeChildMappings = (prev, next) => {
  ...
  Object.keys(next).forEach((nextKey) => {
    if (nextKeysPending[nextKey]) {
      nextKeysPending[nextKey].forEach((pendingNextKey) => {
...
const getProp = (child, prop, props) => {
  return props[prop] != null ? props[prop] : child.props[prop];
};

export const getInitialChildMapping = (props, onExited) => {
...
export const getNextChildMapping = (nextProps, prevChildMapping, onExited) => {
  ...
  Object.keys(children).forEach((key) => {
    ...
    const prevChild: any = prevChildMapping[key];
